...
a = -1;		b = 1;		% x-domain
c = -1;		d = 1;		% y-domain (needed if spDim = 2)

%a-variable 
alpha_min = 0;   alpha_max = 1;       % alpha-level bounds
alpha_num_levels = 9;           % number of a-levels to discretise
...
CC_IC = zeros(ny,nx);
CC_IC_shift = zeros(ny,nx);

epsilon = 0.3;
for i=1:ny
    for j =1:nx
        CC_IC(i, j) =  100*exp(-(xc(i)^2+yc(j)^2)/epsilon);
        CC_IC_shift(i, j) =  100*exp(-((xc(i)-0.8)^2+(yc(j)-0.8)^2)/epsilon);
        if xc(j)^2+yc(i)^2 > 0.01
            CC_IC(i, j) = 0;
        end
        if (xc(j)-0.8)^2+(yc(i)-0.8)^2 > 0.1
            CC_IC_shift(i, j) = 0;
        end
    end
end
CC_IC = CC_IC - min(CC_IC(:));
% clf;    imagesc(CC_IC_shift), colorbar, pause
% ECM_IC = 1 - CC_IC;
...
[ECM_IC, ~, ~] = TerrainNew( log2(size(CC_IC,1)), 2);

% rescale it between the min and max of the ECM that you previously extracted
% for i=1:nx
%     for j =1:ny
%         ECM_IC(i,j) = xc(i)+yc(j);
%     end
% end

% clf; imagesc(ECM_IC); colorbar; pause
...
CC_IC = 7e-2* CC_IC;   % 1st component of S matrix
CC_IC_shift =7e-2*CC_IC_shift;
GF_IC = 8e-2*CC_IC/max(CC_IC(:));    % 2nd component of S matrix
%the FIB and the ECM share the same tissue 
% FIB_IC = 0.8*ECM_IC;    % 3nd component of S matrix
% ECM_IC(:) = 0.2;
% clf; imagesc(ECM_IC); axis square; colorbar;
% pause
...
CC_IC_all(: , : , 1) = CC_IC; 
% CC_IC_all(: , : , 5) = CC_IC; 
% CC_IC_all(: , : , 9) = CC_IC_shift ;
clear CC_IC CC_IC_shift
