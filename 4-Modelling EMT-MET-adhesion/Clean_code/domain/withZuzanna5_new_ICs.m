...
a = -3;		b = 3;		% x-domain
c = -3;		d = 3;		% y-domain (needed if spDim = 2)

%a-variable 
alpha_min = 0;   alpha_max = 1;       % alpha-level bounds
alpha_num_levels = 5;           % number of a-levels to discretise
...
for i=1:nx
    for j =1:ny
        ...
        % ECM_IC(i, j) = xc(i) + yc(j);%1 - CC_IC(i, j);
    end
end
...
[ECM_IC, ~, ~] = TerrainNew( log2(size(CC_IC,1)), 1);
...
%setup the IC matrix for all levels of alpha
