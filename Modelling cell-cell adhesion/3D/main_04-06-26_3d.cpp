...
int main(){
    ...
    for(double t=0; t<=t_max/dt; t++){
        ...
        for(int idx=0; idx<n_cells; idx++){
            ...
            double det = M_xx*(M_yy*M_zz-M_yz*M_yz)-M_xy*(M_xy*M_zz-M_xz*M_yz)+M_xz*(M_xy*M_yz-M_xz*M_yy);
            M_xx+=1;
            M_yy+=1;
            M_zz+=1;
        ...
        if(int(t)%t_save==0 && t!=0){
            ...
            filename << "vtk/cells_" << std::setw(4) << std::setfill('0') << frame << ".vtk";
