...
int main(){
    ...
    for(double t=0; t<=t_max/dt; t++){
        ...
        vector<double> bruit_x(n_cells), bruit_y(n_cells);
        for(int i=0; i<n_cells; i++){
            bruit_x[i]=dist(gen);
            bruit_y[i]=dist(gen);
        }
        ...
        for(int idx=0; idx<n_cells; idx++){
            ...
            for(int k=0; k<neigh[idx].size(); k++){
                ...
                if(d<2*R){ // Cells are touching
                    ...
                    if(cells[idx].is_invasive){
                        cells[idx].force_x += bruit_x[idx];
                        cells[idx].force_y += bruit_y[idx];
                    }
                }
            ...
            // We force beta > CT
            if((t==1/dt && idx==nx*ny/2+24) || (t==3/dt && idx==nx*ny/4+11) || (t==10/dt && idx==nx*ny*7/8+43)){
            ...
            if (cells[idx].is_invasive && ((t*dt)-cells[idx].t_invasive)>=5){
