...
int main(){
    // Model variables
    double *n, *f, *m, *m_new, *c, *c_new, *omega_conso, *a, *a_new, *v;
    ...
    for(int t_step=0; t_step<t+1; t_step++){
        ...
        for(int j=1; j<ny-1; j++){
            for(int i=1; i<nx-1; i++){
                int ij = i + j*nx;
                if(v[ij]==0 && (v[ij-1]==1|| v[ij-nx-1]==1|| v[ij+nx-1]==1)){
                    double prob_new=p_angio*a[ij];
                    if(loop(gen)<prob_new) nouveaux_vaisseaux.push_back(ij);
                }
            }
        }
        for(int ij : nouveaux_vaisseaux){
            v[ij]=1;
            c[ij]=1;
        }
