...
const double rho=0.001, nu=0.025, omega=0.57, phi=0.025;
...
const double omega0 = omega/4.0;
const double kappa0 = 0.0;
const double it_type0 = (20.0/16.0)*(1.0/k);

int main(){
    // Model variables
    double *n, *f, *m, *m_new, *c, *c_new, *omega_conso;
    ...
    random_device rd;
    mt19937 gen(rd());
    std::uniform_int_distribution<int> ic(195, 205);
    std::uniform_real_distribution<double> loop(0, 1);
    std::uniform_int_distribution<int> proliferate(1,4);
    std::uniform_int_distribution<int> age_rand(0,it_type0);
    ...
    for(int j=1; j<ny-1; j++){
        for(int i=1; i<nx-1; i++){
            ...
            if(r2<0.01 && n[ij]==0){
                ...
                cells.push_back({i, j, new_age, id_nb, 0});
                n[ij] += 1;
                id_nb += 1;
            }
        }
    ...
    for(int t_step=0; t_step<t+1; t_step++){
        ...
        for(auto it = cells.begin(); it != cells.end(); ++it){
            ...
            if(it->type==0){kappa_type = kappa0; omega_type = omega0;}
            else if(it->type==1){kappa_type = kappa1; omega_type = omega1;}
        ...
        for(int idx=0; idx < cells.size(); idx++){
            ...
            int it_age = 0;
            if(cells[idx].type==0) it_age=it_type0;
    ...
    std::system("python -m plotter_2d_IBM_gauss_param.py");
    cout << "Fin" << endl;
