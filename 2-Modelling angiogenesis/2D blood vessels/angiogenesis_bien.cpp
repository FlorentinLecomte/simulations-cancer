...
// Model parameters
const double dn=0.0005, dm=0.0005, dc=0.005, dcell=0.0025, dv=0.00001;
...
const double chi=1, rho5 = 0.34, alpha=1;
    ...
    for(int t_step=0; t_step<t+1; t_step++){
        ...
        if(t_step>=10*2000){ // We wait for TAF to be diffused in the cell
            ...
            for(auto it=vessels.begin(); it!= vessels.end(); it++){
                ...
                it->age+=k;
                if(it->age>=0.2){
                    double prob_branch=a[ij]*1; // Depend on TAF concentration
                    if(loop(gen)<prob_branch){
                        new_vessels.push_back({it->i, it->j, 0, true, it->old_i, it->old_j});
                        it->age=0;
                    }
                }
            }
