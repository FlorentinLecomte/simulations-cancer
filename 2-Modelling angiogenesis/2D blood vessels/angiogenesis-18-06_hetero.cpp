...
int main(){
    ...
    for(int j=0; j<ny; j++){
        for(int i=0; i<nx; i++){
            ...
            double x =(i+0.5)*hx;
            double y =(j+0.5)*hy;
            double onde1 = sin(2.6*numbers::pi*x);
            double onde2 = cos(2.4*numbers::pi*y);
            double onde3 = sin(5*numbers::pi*(x +0.5*y));
            double valeur_brute = onde1 + onde2 + onde3;
            f[ij] = 0.5 + valeur_brute/10;
            a[ij]=0;
