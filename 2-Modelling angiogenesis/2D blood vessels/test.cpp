...
struct SproutTip {
    ...
    double age;
    bool active;
    int last_i; // Mémoire de la case précédente
    int last_j;
};
...
int main(){
    ...
    //         // Gradient de fibronectine (plus concentré près du vaisseau)
    //         f[ij] = 0.75 * exp(- (x * x) / 0.45);

    //         // Gradient de TAF pré-calculé (la tumeur est environ à x=0.8)
    //         // Cela simule le fait que le TAF a déjà diffusé jusqu'aux vaisseaux
    //         a[ij] = exp(- (0.8 - x)*(0.8 - x) / 0.45);
    ...
    //         else{
    //             v[ij]=0;
    //             c[ij]=1.0 - (double)i/400.0;
    //         }
    //     }
    // }

    vector<SproutTip> sprouts;

    // Indices j correspondant à y = 0.17, 0.3, 0.5, 0.65, 0.84 sur ny=402
    int y_starts[] = {68, 120, 200, 260, 336};

    for(int j_start : y_starts){
        // last_i = 1 et last_j = j_start
        sprouts.push_back({2, j_start, 0.0, true, 1, j_start});

        int ij_start = 2 + j_start * nx;
        v[ij_start] = 1;
        c[ij_start] = 0.1;
    }
    ...
    for(int t_step=0; t_step<t+1; t_step++){
        ...
        for(int iter=0; iter<max_iter; iter++){
            ...
            for(int j=1; j<ny-1; j++){
                for(int i=1; i<nx-1; i++){
                    ...
                    // }
                    // Si la cellule est vivante ET en forte hypoxie (ex: < 0.2 au lieu de 0.4)
                    if(n[ij] > 0 && c_new[ij] < 0.1){
                        // On booste massivement la production de TAF (ex: x10 ou plus)
                        TAF_prod = k * alpha_TAF*20;
                    }
        ...
        // ==========================================
        // DÉPLACEMENT ET RAMIFICATION DES VAISSEAUX
        // ==========================================
        const double D = 0.00035; // Diffusion aléatoire
        const double chi = 0.38;  // Chimiotactisme (TAF)
        const double rho_hapto = 0.34; // Haptotactisme (Fibronectine)

        if(t_step>=8*2000){
            vector<SproutTip> new_sprouts;
            for (auto& tip : sprouts) {
                ...
                double chi_eff = chi / (1.0 + 0.6 * a[ij]);

                // 1. Calcul des probabilités de mouvement selon les gradients
                double P1 = D - (k / (4 * hx2)) * (chi_eff * (a[ij+1] - a[ij-1]) + rho_hapto * (f[ij+1] - f[ij-1]));
                double P2 = D + (k / (4 * hx2)) * (chi_eff * (a[ij+1] - a[ij-1]) + rho_hapto * (f[ij+1] - f[ij-1]));
                double P3 = D - (k / (4 * hy2)) * (chi_eff * (a[ij+nx] - a[ij-nx]) + rho_hapto * (f[ij+nx] - f[ij-nx]));
                double P4 = D + (k / (4 * hy2)) * (chi_eff * (a[ij+nx] - a[ij-nx]) + rho_hapto * (f[ij+nx] - f[ij-nx]));
                double P0 = 1.0 - (P1 + P2 + P3 + P4);

                // if(t_step%2000==0)cout<<"|P0:"<<P0<<"|P1:"<<P1<<"|P2:"<<P2<<"|P3:"<<P3<<"|P4:"<<P4<<endl;

                // Normalisation pour éviter les probabilités négatives
                P1 = max(0.0, P1); P2 = max(0.0, P2); P3 = max(0.0, P3); P4 = max(0.0, P4); P0 = max(0.0, P0);
                double sum = P0 + P1 + P2 + P3 + P4;

                double rand_val = loop(gen) * sum;
                int new_i = i, new_j = j;

                if (rand_val < P0) {}
                else if (rand_val < P0 + P1) { new_i = i - 1; }
                else if (rand_val < P0 + P1 + P2) { new_i = i + 1; }
                else if (rand_val < P0 + P1 + P2 + P3) { new_j = j - 1; }
                else { new_j = j + 1; }

                int new_ij = new_i + new_j * nx;

                // 2. Application du mouvement et Anastomose (fusion)
                if (new_i > 0 && new_i < nx-1 && new_j > 0 && new_j < ny-1) {

                    if (v[new_ij] == 1 && new_ij != ij) {
                        // Vérifier qu'on ne percute pas notre propre case précédente
                        if (new_i == tip.last_i && new_j == tip.last_j) {
                            // On annule le mouvement, on reste sur place ce tour-ci
                            new_i = i;
                            new_j = j;
                            new_ij = ij;
                        } else {
                            // Vraie anastomose avec un autre vaisseau !
                            tip.active = false;
                            v[new_ij] = 1;
                            c[new_ij] += 0.1;
                        }
                    ...
                    // Si le vaisseau a bougé et est toujours actif
                    if (new_ij != ij && tip.active) {
                        tip.last_i = i; // Mise à jour de la mémoire
                        tip.last_j = j;
                        tip.i = new_i;
                        tip.j = new_j;
                        v[new_ij] = 1; // Avance et laisse le sillage
                        c[new_ij] += 0.1;
                    }
                }

                // 3. Ramification (Branching)
                tip.age += k;

                // On réduit l'âge de maturité (ex: de 0.5 à 0.25) pour qu'ils puissent
                // se ramifier plus souvent une fois proches de la tumeur.
                if (tip.age >= 0.1) {

                    // On augmente drastiquement la sensibilité au TAF (ex: 0.2 au lieu de 0.05)
                    // Tu peux même essayer a[ij] * a[ij] pour un effet exponentiel !
                    double prob_branch = a[ij]*0.5;

                    if (loop(gen) < prob_branch) {
                        new_sprouts.push_back({tip.i, tip.j, 0.0, true, tip.last_i, tip.last_j});
                        tip.age = 0.0;
                    }
                }
            }
            sprouts.insert(sprouts.end(), new_sprouts.begin(), new_sprouts.end());
        }
